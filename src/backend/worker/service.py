...
def shell_wrapper(cmd: str, cwd: pathlib.Path = None, comment: str = None) -> str:
    if comment:
        logger.info(comment)
    logger.debug(cmd)
    result = subprocess.run(cmd, cwd=cwd, capture_output=True, shell=True, text=True, check=True)
...
def build(fuzzer: str, target: str):
    ...
    shell_wrapper(cmd, CAPTAIN_PATH, f'Building "{get_image_name(fuzzer, target)}" image...')


def get_workdir_and_shared(tmp_workdir: pathlib.Path = None) -> Tuple[pathlib.Path, str]:
...
def start(fuzzer: str, target: str, program: str, shared: str, poll: int, timeout: str):
    cmd = f'FUZZER={fuzzer} TARGET={target} PROGRAM={program} SHARED={shared} POLL={poll} TIMEOUT={timeout} ./start.sh'
...
def run_fuzz_pipeline(campaign_id: str, pipeline_id: str, fuzzer: str, target: str, program: str, shared: str,
                      workdir: pathlib.Path, poll: int, timeout: str, storage: Storage):
    """
    ...
    :param shared: shared path(volume) between host(container) and fuzzing container
    :param workdir: local to worker path with the shared results from the fuzzing container
    ...
    with tempfile.TemporaryDirectory(pipeline_id, campaign_id) as tmp_dir:
        ...
        object_name = f'{campaign_id}/{fuzzer}/{target}/{program}/{pipeline_id}/{tar.name}'
        storage.put(tar, object_name)
        return object_name
...
def reduce(campaign_id, fuzz_storage: Storage, reports_storage: Storage):
    with tempfile.TemporaryDirectory(prefix=campaign_id) as tmp_dir:
        ...
        cmd = f'python {BENCHD_PATH / "exp2json.py"} {workdir.absolute()} {report.absolute()}'
